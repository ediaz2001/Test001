/*** Object: StoredProcedure [Erp].[_ZFW_LaborLaborDtlAttch_GetBySysRowID] ******/

CREATE OR ALTER PROCEDURE [Erp].[_ZFW_LaborLaborDtlAttch_GetBySysRowID]
...
BEGIN
	...
	SET NOCOUNT ON;
---------------------------------------------------------------------
-- SELECT for table XFileAttch AS LaborDtlAttch / Table Number : 1
---------------------------------------------------------------------
SELECT [P].[Company],
[P].[LaborHedSeq],
[P].[LaborDtlSeq],
...
JOIN [Erp].[LaborDtl] AS [P] ON ([P].[SysRowID] = [t0].[ForeignSysRowID])
