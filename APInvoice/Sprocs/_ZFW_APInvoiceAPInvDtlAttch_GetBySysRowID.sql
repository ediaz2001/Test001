/*** Object: StoredProcedure [Erp].[_ZFW_APInvoiceAPInvDtlAttch_GetBySysRowID] ******/

CREATE OR ALTER PROCEDURE [Erp].[_ZFW_APInvoiceAPInvDtlAttch_GetBySysRowID]
...
BEGIN
	...
	SET NOCOUNT ON;
---------------------------------------------------------------------
-- SELECT for table XFileAttch AS APInvDtlAttch / Table Number : 1
---------------------------------------------------------------------
SELECT [P].[Company],
[P].[VendorNum],
[P].[InvoiceNum],
...
JOIN [Erp].[APInvDtl] AS [P] ON ([P].[SysRowID] = [t0].[ForeignSysRowID])
